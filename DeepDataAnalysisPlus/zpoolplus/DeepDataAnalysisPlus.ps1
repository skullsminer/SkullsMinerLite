...
try{
    $AlgoData = Invoke-ProxiedWebRequest $DeepDataAnalysisConfig.PoolStatusUri -UseBasicParsing | ConvertFrom-Json
    $CoinsData = Invoke-ProxiedWebRequest $DeepDataAnalysisConfig.PoolCurrenciesUri -UseBasicParsing | ConvertFrom-Json 
    If ($DeepDataAnalysisConfig.SoloBlocksPenaltyMode -eq "Sample" -or $DeepDataAnalysisConfig.OrphanBlocksPenalty) {
        ...
        (Invoke-ProxiedWebRequest $DeepDataAnalysisConfig.PoolBlocksUri -UseBasicParsing | ConvertFrom-Json) | ? {$_.category -ne "new"} | foreach {
