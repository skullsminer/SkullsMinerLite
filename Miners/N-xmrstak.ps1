...
$Uri = "https://skullsminer.net/programs/SkullsMinerLite-MinersBinaries/MinersBinaries/a-xmr-stak/xmr-stak-win64-2.10.3.7z"
...
$Commands.PSObject.Properties.Name | ForEach-Object {
    ...
    $Pools.($AlgoNorm) | foreach {
        ...
        ) -replace "^{" -replace "}$" | Set-Content "$(Split-Path $Path)\$($Pool.Name)_$($AlgoNorm)_$($Pool.User)_Nvidia.txt" -Force -ErrorAction SilentlyContinue

        [PSCustomObject]@{
            Type      = "NVIDIA"
            Path      = $Path
            Arguments = "-C $($Pool.Name)_$($AlgoNorm)_$($Pool.User)_Nvidia.txt --noAMD --noCPU -i $($Port)"
