...
Try {
    ...
    if (Test-Path ((split-path -parent (get-item $script:MyInvocation.MyCommand.Path).Directory) + "\DeepDataAnalysisPlus\ahashpoolplus\ahashpoolplus.xml")) {
        $dtAlgos.ReadXml((split-path -parent (get-item $script:MyInvocation.MyCommand.Path).Directory) + "\DeepDataAnalysisPlus\ahashpoolplus\ahashpoolplus.xml") | out-null
    }
}
catch { return }
