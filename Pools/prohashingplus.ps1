...
Try {
    ...
    if (Test-Path ((split-path -parent (get-item $script:MyInvocation.MyCommand.Path).Directory) + "\DeepDataAnalysisPlus\prohashingPlus\prohashingPlus.xml")) {
        $dtAlgos.ReadXml((split-path -parent (get-item $script:MyInvocation.MyCommand.Path).Directory) + "\DeepDataAnalysisPlus\prohashingPlus\prohashingPlus.xml") | out-null
    }
}
catch { return }
