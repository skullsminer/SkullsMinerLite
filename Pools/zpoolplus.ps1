...
Try {
    ...
    if (Test-Path ((split-path -parent (get-item $script:MyInvocation.MyCommand.Path).Directory) + "\DeepDataAnalysisPlus\zpoolplus\zpoolplus.xml")) {
        $dtAlgos.ReadXml((split-path -parent (get-item $script:MyInvocation.MyCommand.Path).Directory) + "\DeepDataAnalysisPlus\zpoolplus\zpoolplus.xml") | out-null
    }
}
catch { return }
